...
exports.withdrawFromPlan = exports.createSavingsPlan = exports.getUserPlans = exports.Principal = void 0;
...
// src/savingvault_backend/src/savings_vault.ts
var savingsPlans = new StableBTreeMap(0, 44, 1024);
...
function createSavingsPlan(input) {
    ...
    const durationNanos = input.duration * BigInt(24) * BigInt(60) * BigInt(60) * BigInt(1000000000);
    const newPlan = {
        id: planId,
        owner: ic.caller(),
...
function withdrawFromPlan(planId) {
    ...
    const currentTime = ic.time();
    let withdrawalAmount = plan.amount;
    if (plan.planType.kind === "Fixed" && currentTime < plan.maturityDate) {
        const interestEarned = calculateInterest(plan);
        withdrawalAmount += interestEarned / BigInt(2);
        return Result.Err({
            kind: "WithdrawalBeforeMaturity",
            message: "Early withdrawal will incur penalties"
        });
    }
    withdrawalAmount += calculateInterest(plan);
    const updatedPlan = _objectSpread({}, plan, {
        isActive: false
    });
    savingsPlans.insert(planId, updatedPlan);
    return Result.Ok(withdrawalAmount);
}
exports.withdrawFromPlan = withdrawFromPlan;
function generatePlanId() {
    return `PLAN-${ic.time().toString()}`;
}
function calculateInterestRate(planType, duration) {
    if (planType.kind === "Fixed") {
        return BigInt(500) + duration * BigInt(100);
    }
    return BigInt(300);
}
function calculateInterest(plan) {
    const currentTime = ic.time();
    const timeElapsed = currentTime - plan.startTime;
    const yearInNanos = BigInt(365) * BigInt(24) * BigInt(60) * BigInt(60) * BigInt(1000000000);
